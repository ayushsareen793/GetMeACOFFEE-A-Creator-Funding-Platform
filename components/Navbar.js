...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="flex flex-col md:flex-row justify-between items-center px-10 h-18 z-50 bg-black sticky top-0 border-b-2 border-purple-600">

        {/* logo */}
        <Link className="flex items-center gap-3 no-underline" href="/">
          <div className="w-11 h-11 bg-purple-600 flex items-center justify-center shrink-0 [clip-path:polygon(0_0,calc(100%-10px)_0,100%_10px,100%_100%,0_100%)]">
        ...
        <div className="flex items-center gap-3 relative w-full md:w-auto justify-center md:justify-end">
          ...
          {session && (
            <div ref={dropdownRef}>
              ...
              {/* Dropdown */}
              <div className={`absolute right-0 top-[calc(100%+8px)] w-48 border border-purple-600/30 bg-black shadow-[0_20px_60px_rgba(147,51,234,0.15)] z-999 p-1.5 [clip-path:polygon(0_0,calc(100%-10px)_0,100%_10px,100%_100%,0_100%)] ${showdropdown ? "block" : "hidden"}`}>
